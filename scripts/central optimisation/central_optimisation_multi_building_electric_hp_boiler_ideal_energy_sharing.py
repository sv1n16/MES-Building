...
pv_data = pd.read_csv(
    "data\\pv_generation_2018_02_21.csv"
)

# Add PV to main dataframe
...
irradiance = data_hr["pv"].values.flatten() 
pv_data = irradiance
...
def electricity_balance_rule(model, b, t):
    ...
    energy_received = sum(
        model.energy_share[i, b, t]
        for i in model.buildings
        if i != b
    )

    energy_sent = sum(
        model.energy_share[b, j, t]
...
def energy_export_limit_rule(model, b, t):
    ...
    # Energy sold can only come from PV or battery discharge
    return energy_sent <= (
        model.pv_supply[b,t]
        + model.discharge[b,t]
    )
...
model.energy_export_limit = pyo.Constraint(
    model.buildings,
    model.t,
    rule=energy_export_limit_rule
)


def no_self_sharing_rule(model,b,t):
    return model.energy_share[b,b,t] == 0

model.no_self_sharing = pyo.Constraint(
    model.buildings,
    model.t,
    rule=no_self_sharing_rule
)

def energy_import_limit_rule(model, b, t):
    ...
    # energy received cannot exceed consumption + charging + HP demand
...
model.energy_import_limit = pyo.Constraint(
    model.buildings,
    model.t,
    rule=energy_import_limit_rule
)
...
def objective_rule(model):
    ...
    for b in model.buildings:
        for t in model.t:
            ...
            gas_cost = (
                gas_price / 100
                * model.gas_consumption[b, t]
                * dt
            )

            comfort_penalty = (
                alpha
                * (model.T_in[b, t] - model.T_set[b, t]) ** 2
            )
...
result = solver.solve(
    model,
    tee=True,
    logfile="Results/Temp_setpoint/solver_log.txt"
)
...
sharing_schedule = np.array([
    [
        [
            pyo.value(model.energy_share[i, j, t])
            for t in model.t
        ]
        for j in model.buildings
    ]
...
energy_sold = sharing_schedule.sum(axis=1)      # (building, time)
energy_bought = sharing_schedule.sum(axis=0)    # (building, time)

# Convert to monetary value (£)
energy_sold_value = np.zeros((n_buildings, time_horizon))
energy_bought_value = np.zeros((n_buildings, time_horizon))

community_shared_total = sharing_schedule.sum(axis=(0, 1))
total_sold_check = sharing_schedule.sum()
total_bought_check = sharing_schedule.sum()  # identical by construction (bilateral variable)
print("\n" + "=" * 70)
print("GLOBAL COMMUNITY SHARING CHECK")
print("=" * 70)
print(f"Total sold across horizon (kWh): {total_sold_check:.6f}")
print(f"Total bought across horizon (kWh): {total_bought_check:.6f}")
print(f"Total exchanged energy == 0: {bool(abs(total_sold_check-total_bought_check) <= 1e-6)}")
print(f"Max energy shared community-wide at any single timestep (kWh): {community_shared_total.max():.6f}")

for b in range(n_buildings):
    for t in range(time_horizon):
        price = data_hr["price"].values[t]

        energy_sold_value[b, t] = (
            0.8 * price * energy_sold[b, t]
        )

        energy_bought_value[b, t] = (
            0.8 * price * energy_bought[b, t]
        )
for b in model.buildings:
    row = b + 1
...
fig.show()
# Community-level sharing diagnostics

fig_sharing_total = go.Figure()
fig_sharing_total.add_trace(
    go.Scatter(
        x=list(range(time_horizon)),
        y=community_shared_total,
        mode="lines",
        name="Total energy shared (kWh)",
        line=dict(color="teal", width=2),
    )
)
fig_sharing_total.update_layout(
    title="Total Community-Wide Energy Shared per Timestep",
    xaxis_title="Time",
    yaxis_title="Total energy shared (kWh)",
    template="plotly_white",
)
fig_sharing_total.show()

# ============================================================================
# ENERGY SOURCES PER BUILDING
# ============================================================================

for b in range(n_buildings):

    # Energy received from each other building
    received_from_buildings = {}

    for source in range(n_buildings):
        if source != b:
            received_from_buildings[f"Building {source}"] = sharing_schedule[source, b, :]
    ...
    # Community energy received
    for source_name, energy in received_from_buildings.items():
        fig_sources.add_trace(
            go.Scatter(
                x=list(range(time_horizon)),
                y=energy,
    ...
    fig_sources.update_layout(
        title=f"Building {b}: Electricity Supply Sources",
        xaxis_title="Time (hours)",
        yaxis_title="Electricity supplied (kW)",
        template="plotly_white",
        hovermode="x unified",
    )
...
for b in model.buildings:
    ...
    print(
        f"Building {b}: Total electricity cost = £{electricity_cost_b:.2f}, "
