...
alpha = 0
gas_price = 1.0  # p/kWh, example value, replace with actual
# ---- Parameters ----
...
model.electric_load = pyo.Param(
    model.buildings,
    model.t,
    initialize={(b, t): float(data[cols[b]].values[t]) / 1000 for b in model.buildings for t in model.t},
)
model.p_el_vars = pyo.Var(model.buildings, model.t, bounds=(0, None))  # imported electricity from grid

model.p_hp = pyo.Var(model.buildings, model.t, bounds=(0, p_th_nom))  # imported electricity from grid
...
def thermal_dynamics(
    ...
    return model.T_in[b, t] == model.T_in[b, t - 1] + model.dt / 10 * (
        (model.q_heat_vars[b, t]) - 0.5 * (model.T_in[b, t - 1] - model.T_out[b, t])
    )
...
def objective(model):
    return sum(
        data_hr["price"][t] * (model.p_el_vars[b, t]) * model.dt + alpha * (model.T_in[b, t] - model.T_set[b, t]) ** 2
        for b in model.buildings
        for t in model.t
    )
