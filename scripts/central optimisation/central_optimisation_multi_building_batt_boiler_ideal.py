...
top3_consumers = consumption_data.sum().sort_values(ascending=False).head(3).index.tolist()
...
alpha = 0  # Temperature deviation penalty weight

# ---- Parameters ----

irradiance = data_hr["pv"].values.flatten() / 1000
pv_data = irradiance
# PV supply for each building and time (assuming same PV for all buildings)
...
# Boiler variables only (no heat pump)
...
def gas_consumption_rule(model, b, t):
    """Gas consumption from boiler output: gas_in = boiler_out / efficiency"""
    if efficiency > 0:
        return model.gas_consumption[b, t] == model.q_boiler_vars[b, t] / efficiency
    else:
        return model.gas_consumption[b, t] == 0
...
def electricity_balance_rule(model, b, t):
    """Electricity balance: grid import = load + charging - PV - discharging"""
    return model.p_el_vars[b, t] == (
        model.electric_load[b, t] + model.charge[b, t] - model.pv_supply[b, t] - model.discharge[b, t]
    )
...
def heat_balance_rule(model, b, t):
    """Heat balance: all heat comes from boiler"""
    return model.q_boiler_vars[b, t] == model.q_heat[b, t]
...
def thermal_dynamics_rule(model, b, t):
    """Building thermal dynamics: dT/dt = (q_heat - heat_loss) / C"""
    if t == 0:
...
T_in_all = np.array([[pyo.value(model.T_in[b, t]) for t in model.t] for b in model.buildings])
T_out_all = np.array([[pyo.value(model.T_out[b, t]) for t in model.t] for b in model.buildings])
T_set_all = np.array([[model.T_set[b, t] for t in model.t] for b in model.buildings])
q_boiler_schedule = np.array([[pyo.value(model.q_boiler_vars[b, t]) for t in model.t] for b in model.buildings])
q_heat_schedule = np.array([[pyo.value(model.q_heat[b, t]) for t in model.t] for b in model.buildings])
gas_schedule = np.array([[pyo.value(model.gas_consumption[b, t]) for t in model.t] for b in model.buildings])
...
fig = make_subplots(
    rows=n_buildings,
    cols=3,
    subplot_titles=(f"Battery Operation", "Building Temperatures", "Costs"),
    shared_xaxes=True,
    shared_yaxes=False,
)
...
for b in model.buildings:
    ...
    print(f"\nBuilding {b}:")
    print(f"  Electricity cost: £{elec_cost_b:.2f}")
    print(f"  Gas cost: £{gas_cost_b:.2f}")
    print(f"  Total cost: £{total_b:.2f}")
...
fig.update_layout(
    ...
    width=1400,
    title_text=f"Multi-Building Energy Optimization (Boiler Only, alpha={alpha})",
...
print("=" * 80)
print(
    f"Total electricity cost: £{sum([sum([data_hr['price'].values[t] * (pyo.value(model.charge[b, t]) - pyo.value(model.discharge[b, t]) - pyo.value(model.pv_supply[b, t]) + model.electric_load[b, t]) for t in model.t]) for b in model.buildings]):.2f}"
)
print(
    f"Total gas cost: £{sum([sum([gas_price / 100.0 * pyo.value(model.gas_consumption[b, t]) for t in model.t]) for b in model.buildings]):.2f}"
)
