...
def objective(model):
    return sum(
        price[t] * (model.p_el_vars[b, t]) * model.dt + (0 * (model.T_in[b, t] - model.T_set[b, t]) ** 2)
        for b in model.buildings
        for t in model.t
    )
    ...
    [
        [
            (
                ...
                + pyo.value(model.electric_load[b, t])
...
print("heating output:")
print(q_heatpump_schedule)
