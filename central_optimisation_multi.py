...
building_parameters = {
    ...
    "thermal": {
        ...
        "insulation_level": "high",  # insulation_level: str – 'good', 'average', or 'poor'
    },
    "pv": {
...
bd = Building(building_components=[bat, pv, hp, boiler])
# Gurobi model
model = pyo.ConcreteModel()
model.t = pyo.RangeSet(0, time_horizon - 1)


n_buildings = 3  # Set this to the number of buildings you want to model
...
def objective(model):
    return sum(
        price[t]
        * (load[b, t] - pv[b].p_el_supply[t] + model.charge[b, t] - model.discharge[b, t] + model.p_el_vars[b, t])
        * delta_t
        + boiler[b].gas_price * model.gas_consumption[b, t] * delta_t
...
fig.add_trace(go.Scatter(x=plot_time, y=boiler_thermal_output, name="Boiler Thermal Output (kWh)"), row=2, col=2)
fig.add_trace(go.Scatter(x=plot_time, y=thermal_load_setpoint, name="Thermal Load (kWh)"), row=2, col=2)
fig.add_trace(go.Scatter(x=plot_time, y=hp_electric_consumption, name="HP Electrical Consumption (kWh)"), row=3, col=2)
fig.add_trace(go.Scatter(x=plot_time, y=boiler_gas_consumption, name="Boiler Gas Consumption (kWh)"), row=4, col=2)

# Add costs and consumption traces to the right column
...
fig.add_trace(go.Scatter(x=plot_time, y=boiler_gas_consumption, name="Gas Consumption (kWh)"), row=9, col=2)
